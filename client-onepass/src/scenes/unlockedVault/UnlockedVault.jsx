...
import { addUserLogin, deleteVaultLogin } from "../../actions/loginActions";
...
const UnlockedVault = () => {
    ...
    useEffect(() => {
        if (searchText !== "") {
            const resultsArray = vaultLogins?.filter(
                (login) =>
    ...
    useEffect(() => {
        if (page === 1 && page > 0) {
            ...
            if ((page - 1) * tableRows + tableRows > vaultLogins?.length) {
                const loginArr = vaultLogins?.slice(
                    (page - 1) * tableRows,
                    vaultLogins?.length
                );
                setLoginList(loginArr);
                    ...
                    <>
                        ...
                        >
                            {isNonMobile ? (
                                <IconButton
                                    onClick={() =>
                                        handleTickClick(customFieldSize)
                                    }
                                >
                                    <MdOutlineDone
                                        style={{
                            ...
                            >
                                ...
                                >
                                    <motion.button
                                        ...
                                        whileHover={{ scale: [1, 1.1] }}
                                        onClick={() =>
                                            handleAddCustomFieldButtonClick()
                                        }
                                    >
                                        Add Custom Field
                                    </motion.button>
                ...
                <Row style={{ marginTop: "1rem", marginBottom: "1rem" }}>
                    {
                        <>
                            ...
                            {selectedLogin?.customFields?.length > 0 ? (
                                selectedLogin?.customFields.map(
                                    (field, index) => (
                                        <Col md={6} key={index}>
                                            <Typography
                                                sx={{
    ...
    // Remove credentials
    const [showLoginRemoveModal, setShowLoginRemoveModal] = useState(false);
    const [loginTodelete, setLoginTodelete] = useState(undefined);

    const removeButtonClickHandle = (login) => {
        setLoginTodelete(login);
        setShowLoginRemoveModal(true);
    };

    const closeLoginRemoveModal = () => {
    ...
    const renderLoginTable = () => {
        return (
            ...
            >
                <Table
                    striped
                    style={{ fontSize: 14, alignItems: "", height: "100%" }}
                    responsive
                >
                    <thead>
                        <tr>
                            ...
                            >
                                ...
                                >
                                    <FlexBetween
                                        ...
                                        gap="3px"
                                        padding={
                                            isNonMobile ? "0.1rem 1.5rem" : null
                                        }
                                        paddingLeft={
                                            isNonMobile ? null : "1rem"
                                        }
                                    >
                                        <InputBase
                                            placeholder="Search..."
                                        ...
                                        >
                                            <Typography
                                                ...
                                                sx={{
                                                    ...
                                                    color: "transparent",
                                                    backgroundImage:
                                                        "linear-gradient(to left, #6d4aff, #cc00ee)",
                                                    backgroundSize: "100%",
                                                    backgroundClip: "text",
                                                    backgroundRepeat: "repeat",
                                                }}
